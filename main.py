...
def ai_command(body: AiIn):
    ...
    if body.mode != "quick":
        ...
        if body.form:
            try:
                result = execute_ops(STORE, _quick_form_ops(intent))
                result["engine"] = "form"
...
def ai_plan(body: AiPlanIn):
    ...
    intent = build_intent(text=text, model=STORE.ensure_loaded())
    ...
    try:
        ...
        out = run_cad_plan(
            ...
            sys_p=sys_p, user_base=build_user_message(text, intent=intent))
        if not out["ok"]:
        ...
        print(f"[ai-plan] provedor falhou: {e}", flush=True)
