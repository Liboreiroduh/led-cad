...
class Vec3(BaseModel):
    ...
    def _finite(cls, v: float) -> float:
        # A05: NaN/Infinity corrompem geometria, cotas e exportações —
        # rejeitar na fronteira do modelo, não nas visualizações.
        if v is None or v != v or v in (float("inf"), float("-inf")):
            raise ValueError("Coordenada precisa ser número finito (mm).")
        return v
...
class ProjectModel(BaseModel):
    ...
    def _unique_ids(cls, elements: List["Element"]) -> List["Element"]:
        # A06: IDs duplicados colidem no index() e escondem elementos.
        seen: set = set()
        for e in elements:
            if e.id in seen:
                raise ValueError(f"ID de elemento duplicado: '{e.id}'.")
            seen.add(e.id)
        return elements
