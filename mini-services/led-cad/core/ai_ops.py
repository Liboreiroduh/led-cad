...
def dry_run_diff(model: ProjectModel, ops: List[Any]) -> Dict[str, Any]:
    ...
    raw_ops = list(ops or [])
    # A04: nunca truncar silenciosamente — preview e Apply têm o MESMO
    # limite; excesso é erro explícito ANTES de expandir/simular.
    if len(raw_ops) > 120:
        raise OperationError(
            f"Lote com {len(raw_ops)} operações excede o máximo de 120 "
            "por plano. Divida o pedido ou reduza o escopo.")
